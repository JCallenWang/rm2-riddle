...
- **連筆靠座標貪婪串接**:一筆的起點若落在前一筆的收筆點上就併成同一筆。刻意不去假設
  「哪一筆是字身、哪一筆是附加筆劃」——Hershey 會把一個字母拆成多筆,而且順序不固定
  (`n` 的第一筆是自左 bearing 進來的引筆、第二筆才是字身;`i` 的第一筆卻是上面那個點),
  依筆劃順序猜角色會猜錯,實測會讓連筆完全失效。實測合併後一行約 29 筆(未合併約 40 筆)。
